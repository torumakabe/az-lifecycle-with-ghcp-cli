...
- **Azure CLI の `rdbms-connect` 拡張を事前にインストールしておく** — `postprovision.py` は `az postgres flexible-server execute` を**常に最初に試行**する実装になっており、この CLI が使えない場合は即失敗する（`NetworkReachabilityError` ではなく `CalledProcessError` として伝播するため、`psql` フォールバックにも入らない）。初回デプロイ前に以下を実行しておくこと:

  ```shell
  az extension add --name rdbms-connect --yes
  az extension show --name rdbms-connect --query version -o tsv  # インストール確認
  ```

  `rdbms-connect` は `psycopg2-binary` / `jsonschema` / `rpds-py` などを含み、Azure CLI の Python 環境に依存モジュールをインストールする。Homebrew 版 Azure CLI など一部環境で `No module named 'rpds'` 等の依存不足が発生した場合は、Azure CLI の Python で `rpds-py` を手動インストール（`$(dirname "$(readlink -f "$(command -v az)")")/../libexec/bin/python -m pip install rpds-py` 相当）して復旧する
