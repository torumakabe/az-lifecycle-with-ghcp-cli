...
- Orphaned sldId entries in p14:sectionLst (PowerPoint 2010+ sections)
...
def remove_orphaned_slides(unpacked_dir: Path) -> list[str]:
    ...
    # Clean orphaned sldId entries from p14:sectionLst in presentation.xml.
    # The sectionLst (PowerPoint 2010+ extension) groups slides into sections.
    # After removing slides from sldIdLst, matching entries in sectionLst must
    # also be removed to prevent PowerPoint from reporting corruption.
    _clean_section_list(unpacked_dir, referenced_slides)

    return removed


def _clean_section_list(unpacked_dir: Path, referenced_slides: set[str]) -> bool:
    """Remove orphaned sldId entries from p14:sectionLst in presentation.xml."""
    pres_path = unpacked_dir / "ppt" / "presentation.xml"
    ...
    # Collect valid numeric slide IDs from the main p:sldIdLst
    valid_ids = set()
    ...
    # Find sectionLst elements (any namespace prefix — p14, p15, etc.)
    for elem in list(dom.getElementsByTagName("*")):
        if elem.localName != "sectionLst":
            continue

        for section in list(elem.childNodes):
            if getattr(section, "localName", None) != "section":
                continue

            for sld_id_lst in list(section.childNodes):
                if getattr(sld_id_lst, "localName", None) != "sldIdLst":
                    continue

                for sld_id in list(sld_id_lst.childNodes):
                    if getattr(sld_id, "localName", None) != "sldId":
                        continue
                    sid = sld_id.getAttribute("id")
                    if sid and sid not in valid_ids:
                        sld_id_lst.removeChild(sld_id)
                        changed = True

            # Remove empty sections (no remaining sldId children)
            remaining = sum(
                1
                for child in section.childNodes
                if getattr(child, "localName", None) == "sldIdLst"
                for grandchild in child.childNodes
                if getattr(grandchild, "localName", None) == "sldId"
            )
            if remaining == 0:
                elem.removeChild(section)
                changed = True
...
def update_content_types(unpacked_dir: Path, removed_files: list[str]) -> None:
    ...
    # PATCH(project): Normalize path separators for cross-platform compatibility.
    # On Windows, removed_files uses backslashes (e.g. ppt\slides\slide1.xml)
    # while PartName uses forward slashes (e.g. /ppt/slides/slide1.xml).
    # Without normalization, no overrides are removed on Windows.
    normalized_removed = {f.replace("\\", "/") for f in removed_files}

    for override in list(dom.getElementsByTagName("Override")):
        part_name = override.getAttribute("PartName").lstrip("/")
        if part_name in normalized_removed:
            if override.parentNode:
                override.parentNode.removeChild(override)
                changed = True

    # PATCH(project): Also remove overrides for files that no longer exist,
    # regardless of how they were deleted. This catches orphans missed by
    # the removed_files list (e.g. files deleted by other cleanup steps).
    for override in list(dom.getElementsByTagName("Override")):
        part_name = override.getAttribute("PartName").lstrip("/")
        file_path = unpacked_dir / Path(part_name)
